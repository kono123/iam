...
euint32 file_fread(File *file,euint32 offset, euint32 size,euint8 *buf)
{
  ...
  if(offset>=file->FileSize) size_left=0; // Offset check
	
  if( (offset+size > file->FileSize) && size_left!=0) size_left=file->FileSize-offset;
	
  while(size_left>0){
	
		cclus = coffset/(512L*file->fs->volumeId.SectorsPerCluster);
		csec = (coffset/(512L))%file->fs->volumeId.SectorsPerCluster;
//		cbyte = coffset%512L;
		cbyte = coffset & 0x1ff;
		
		if(cbyte!=0 || size_left<512){
			btr = 512-cbyte>=size_left?size_left:512-cbyte;
		}else{
			btr = 512;
		}
		...
		if(btr==512){
			...
			memCpy(tbuf+cbyte,buf+bytes_read,btr);
...
//    cbyte = coffset%512L;
    ...
    if(btr==512){
//    printf("==== rsec=0x%lx,csec=0x%lx,rclus=0x%lx\n",rsec,csec,rclus);
      ...
      memCpy(buf+bytes_written,tbuf+cbyte,btr);
      part_relSect(file->fs->part,tbuf);
    }
