...
//	r=if_readBuf(ioman->iface,address,buf);
	r=if_readSector(ioman->iface,address,buf);
...
//	r=if_writeBuf(ioman->iface,address,buf);
	r=if_writeSector(ioman->iface,address,buf);

	if( r!=0 ) {
		ioman_setError(ioman, IOMAN_ERR_WRITEFAIL);
//                printf("writeSector err=0x%x,addr=%lx\n",r,address);
		return(-1);
	}
...
#if 1
	if( ioman->usage[bufplace] == 0 ) {
		ioman_setNotWritable(bufplace);
	}
#else
	/* mth - not used, see sf Bug-Report 1760666 - kept as reference */
...
#else
	/* mth - not used, see sf Bug-Report 1612409 - kept as reference */
